<template>
  <div>
    <h3 style="margin:0 0 20px">商品审核</h3>
    <el-card>
...
async function approve(id) { await approveGoods(id); ElMessage.success('审核通过'); await load() }
function showReject(id) { rejectId = id; rejectReason.value = ''; dialogVisible.value = true }
async function doReject() {
  await rejectGoods(rejectId, rejectReason.value || '未说明原因')
